                            ...
                            const std::string& proxy_domain, int proxy_port)
{
    ...
    detail::http::downloadUrl(newUrl, max_content, respond_headers,
                              downloadFunc, request_header);
#else
    http::downloadUrlCurl(varlisp::detail::http::curl_method_get, newUrl, max_content, respond_headers, request_header, nullptr);
#endif

    if (respond_headers.status_code / 100 != 2) { // 200
        COLOG_ERROR(url);
    ...
    if (respond_headers.has("Content-Type")) {
        // NOTE eg. Content-Type: "image/svg+xml"
        fnameSuffix = "." + sss::path::basename(respond_headers["Content-Type"]);
        trim_to(fnameSuffix, ';');
        trim_to(fnameSuffix, '+');
    }
