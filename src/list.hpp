...
public:
    using shared_t = std::vector<Object>;

    using iterator = shared_t::iterator;
    using const_iterator = shared_t::const_iterator;

    using size_type = shared_t::size_type;

protected:
    List(std::shared_ptr<shared_t> shared, size_t start, size_t len)
        : m_refer(std::move(shared)), m_start(start), m_length(len)
    {}

public:
    void print(std::ostream& o) const;
    void print_impl(std::ostream& o, const List* p = nullptr) const;
